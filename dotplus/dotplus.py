...
def time_cards(credentials):
    ...
    return [parse_work_day(w) for w in time_cards_data['work_days']]
...
def parse_work_day(work_day):
    time_cards = []
    raw_time_cards = work_day['time_cards']
    for raw_time_card in raw_time_cards:
        time_cards += [raw_time_card['time']]

    return {'date': work_day['date'], 'time_cards': time_cards}
